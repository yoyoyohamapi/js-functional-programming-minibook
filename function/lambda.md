...
```
注意后面的归约条件，它表示我们必须在保证自由变量不会与绑定变量起冲突的前提下才能进行归约操作，即保证E'表达式中的自由变量在归约后不会变为E[V:E']表达式的绑定变量。
...
```
**乘法** 运算 `m * n` 实际上就是 `m` 和 `n`  对应函数的幂次相乘，可以看作求函数n次幂的m次幂：
```js
const mutiple = m => n => f => m(n(f));
```
**自减** 运算，其实现比较复杂，为了方便理解，我们先定义一个转移表达式：
```js
